...
Pipeline stages (status updated in Supabase at each step):
  uploaded → extracting → extracted → [generating → complete] (docgen wired in INL-17)
...
class LeaseService:
    """Orchestrates lease upload → extraction → (future: docgen) pipeline."""
    ...
    async def process_lease(
        ...
        Returns a dict with upload_id, status, and extracted_data.
        """
        ...
        try:
            ...
            # ------------------------------------------------------------------
            logger.info("[%s] Stage 1/4: Storing file in Supabase Storage", file_name)
            ...
            logger.info(
                "[%s] Stage 1/4 complete — stored at %s (%.1fs)",
            ...
            logger.info("[%s] Stage 2/4: Extracting text from %s", file_name, file_type.upper())
            stage_start = time.time()
            ...
            logger.info("[%s] Stage 3/4: Sending to Gemini 2.5 Flash for field extraction", file_name)
            stage_start = time.time()
            ...
            # ------------------------------------------------------------------
            logger.info("[%s] Stage 4/4: Saving extracted data to DB", file_name)
            stage_start = time.time()
            ...
            logger.info(
                "[%s] Stage 4/4 complete — data saved (%.1fs)",
                file_name, time.time() - stage_start,
            )

            # ------------------------------------------------------------------
            # Pipeline complete (docgen will be wired in INL-17)
            # ------------------------------------------------------------------
            total_time = time.time() - pipeline_start
            logger.info(
                "[%s] Pipeline complete — status: extracted (%.1fs total)",
